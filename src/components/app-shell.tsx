...
/** Permite que áreas com scroll horizontal (ex.: Kanban) recolham o menu automaticamente ao arrastar. */
export function useSidebarAutoCollapse() {
  const ctx = useContext(SidebarContext);
  return (scrollLeft: number) => {
    if (ctx && scrollLeft > 8) ctx.setCollapsed(true);
  };
}
  ...
  return (
    <SidebarContext.Provider value={{ collapsed, setCollapsed }}>
      <div className="flex min-h-screen bg-background text-foreground">
        ...
        <aside
          className={cn(
            "sticky top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r border-white/5 bg-[#0B0E17] transition-all md:flex",
            collapsed ? "w-20" : "w-64",
          )}
        >
